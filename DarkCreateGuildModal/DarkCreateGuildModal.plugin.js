/**
 * @name DarkCreateGuildModal
 * @author A user
 * @version 1.0.0
 * @description Forces dark theme on the "Create a Server" modal
 */
/*@cc_on
@if (@_jscript)
	...
	(() => {
		...
		var __webpack_modules__ = {
			192: (module, __webpack_exports__, __webpack_require__) => {
				...
				___CSS_LOADER_EXPORT___.push([module.id, ".DarkCreateGuildModal-style-backButtonPatch{color:#fff}", ""]);
				___CSS_LOADER_EXPORT___.locals = {
					backButtonPatch: "DarkCreateGuildModal-style-backButtonPatch"
				};
			...
			676: (__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
				...
				const package_namespaceObject = JSON.parse('{"u":{"u2":"DarkCreateGuildModal"}}');
				...
				const Settings = external_BdApi_React_default().memo((() => external_BdApi_React_default().createElement(external_BdApi_React_default().Fragment, null, external_BdApi_React_default().createElement(SwitchItem, {
					value: settingsManager.get("white-back-button", false),
					onChange: value => settingsManager.set("white-back-button", value)
				}, "Fix the back button by making it white"))));
				var DarkCreateGuildModal_React = __webpack_require__(113);
				class DarkCreateGuildModal extends(external_BasePlugin_default()) {
					onStart() {
						external_StyleLoader_default().inject();
						external_PluginApi_namespaceObject.Patcher.after(external_PluginApi_namespaceObject.WebpackModules.getByProps("CreateGuildSlideTypes"), "default", ((_this, args, res) => {
							res.props.className = `theme-dark`;
						}));
						external_PluginApi_namespaceObject.Patcher.after(external_PluginApi_namespaceObject.WebpackModules.getByProps("ModalFooter"), "ModalFooter", ((_this, args, res) => {
							if (!settingsManager.get("white-back-button", false)) return;
							let backButton = external_PluginApi_namespaceObject.Utilities.findInReactTree(res, (e => e?.props?.look?.indexOf?.("lookBlank") > -1 && e?.props?.size?.indexOf?.("sizeMin") > -1));
							if (!backButton) return;
							backButton.props.className += ` ${style.Z.backButtonPatch}`;
						}));
					}
					getSettingsPanel() {
						return DarkCreateGuildModal_React.createElement(Settings, null);
					}
					onStop() {
