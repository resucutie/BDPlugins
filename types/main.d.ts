/// <reference path="./discord/globals.d.ts" />
/// <reference path="./bdapi.d.ts" />
/// <reference path="./zlib.d.ts" />
/// <reference path="./builder.d.ts" />
/// <reference path="./discord.d.ts" />

/// <reference path="./discord/constants.d.ts" />
/// <reference path="./discord/native.d.ts" />
/// <reference path="./discord/classes.d.ts" />
/// <reference path="./discord/modules.d.ts" />
/// <reference path="./discord/components.d.ts" />
/// <reference path="./discord/icons.d.ts" />

// /// <reference path="./plugins/Fuses.d.ts" />
/// <reference path="./plugins/Timezones.d.ts" />

/// <reference path="./common.d.ts" />

import { ReactFragment } from "react";

declare module "yargs/yargs";